...
export class WorkFlowyClient {
  ...
  createNode = (name, parent_id = "None", position = "top") => {
  ...
  // updateNode = (newName, nodeId) => {
  //   const res = this.request("POST", `/:${nodeId}`, { name: newName });

  //   return res;
  // };

  // getTree = async (parent_id = "None") => {
  //   const res = await this.request("GET", "", { parent_id: parent_id });
  ...
  getUserData = async (treeID = "Root") => {
    const res = await fetch(
      `${API_BASE}/get_initialization_data?client_version=21&client_version_v2=28&no_root_children=1`,
      {
        headers: { cookie: `sessionid=${this.sessionId}` },
      }
    );
    ...
    const lastTransactionIds = {
      Root: userData.mainProjectTreeInfo
        .initialMostRecentOperationTransactionId,
    };

    for (const tree of userData.auxiliaryProjectTreeInfos) {
      lastTransactionIds[tree.shareId] =
        tree.initialMostRecentOperationTransactionId;
    }
    ...
    return {
      ...
      timestamp:
        ...
        userData.mainProjectTreeInfo.dateJoinedTimestampInSeconds,
