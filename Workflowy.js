...
const Workflowy = (function() {
  ...
  Workflowy.prototype.login = function () {
    if (!this.sessionid) {
      ...
      .then(arg => {
        ...
        let jar = this.jar._jar.toJSON()
        console.log("jar", jar)
        for (let i = 0; i < jar.cookies.length; i++) {
          if (jar.cookies[i].key === 'sessionid') {
            this.sessionid = jar.cookies[i].value
            break
          }
        }
      }, err => Q.reject(err))
  ...
  Workflowy.prototype._update = function (operations) {
    return this.meta.then(meta => {
       let clientId, j, len, operation, timestamp
...
// import { run, loadWfConfig, writeWfConfig } from './wf.js';
// export { run as cli, loadWfConfig, writeWfConfig };
